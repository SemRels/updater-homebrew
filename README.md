...
```bash
docker pull ghcr.io/semrels/updater-homebrew:latest
```

Images are signed with [cosign](https://github.com/sigstore/cosign) and include a full SBOM attestation. Verify the signature:

```bash
cosign verify ghcr.io/semrels/updater-homebrew:latest \
  --certificate-identity-regexp 'https://github.com/SemRels/updater-homebrew/.github/workflows/release.yml.*' \
